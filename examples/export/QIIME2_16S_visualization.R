...
#
# 使用方式：
#   1. 将本脚本放入 QIIME2 的 export 目录中
#   2. 确保 metadata.txt 在上级目录 (../metadata.txt) 或修改下方路径
#   3. 在 RStudio 中打开并点击 Source，或命令行运行：
...
#   - 列：样本
#   - 值：丰度（计数）
# 使用 rarefied_table.biom（即 rarefied_table.tsv）作为丰度数据源
# taxonomy.tsv 用于分类学映射，输出时去除表头和 Confidence 列
...
if (file.exists(rarefied_table_file)) {
  ...
  # ASV 到 FullPath 映射（taxonomy 已在第 5 节解析）
  ...
  if (length(common_rare_asvs) > 0) {
    ...
    # 写出 FAPROTAX 输入表（去表头 col.names = FALSE，不含 Confidence 列）
    ...
    cat("       ", nrow(faprotax_df), "个分类群,", ncol(faprotax_df) - 1, "个样本\n")
    ...
    cat("       [OK] faprotax/FAPROTAX_input_table_int.tsv (整数格式)\n")
  } else {
    cat("       [跳过] 稀疏表与分类学无共有 ASV\n")
  }
} else {
  cat("       [跳过] 未找到稀疏表 (", rarefied_table_file, ")\n")
}
...
# 复制 feature-table.biom（PICRUSt2 首选输入）
if (file.exists(biom_file)) {
  ...
  cat("       [OK] picrust2/feature-table.tsv (BIOM 不存在，使用 TSV)\n")
} else {
  cat("       [跳过] 未找到特征表\n")
}
...
cat("输出文件汇总:\n")
cat("========================================\n")
cat("  alpha/alpha_diversity_boxplot.pdf           — α 多样性箱线图（4 指标 × 星号标注）\n")
cat("  alpha/rarefaction_curves.pdf               — 稀释曲线\n")
cat("  beta/beta_diversity_pcoa_bray_curtis.pdf   — β 多样性 Bray-Curtis PCoA（PERMANOVA 标注）\n")
cat("  beta/beta_diversity_pcoa_jaccard.pdf       — β 多样性 Jaccard PCoA（PERMANOVA 标注）\n")
cat("  taxa/phylum_stacked_barplot.pdf              — 门水平堆叠柱状图\n")
cat("  taxa/phylum_abundance_barchart.pdf           — 门水平丰度条形图\n")
cat("  heatmap/genus_heatmap.pdf                    — 属水平热图\n")
cat("  faprotax/FAPROTAX_input_table.tsv            — FAPROTAX 输入表\n")
cat("  picrust2/feature-table.biom                  — PICRUSt2 特征表\n")
cat("  picrust2/dna-sequences.fasta                 — PICRUSt2 代表序列\n")
cat("  feature_tables/taxonomy_processed.tsv        — 分类学层级拆分表\n")
cat("  feature_tables/genus_abundance.tsv           — 属水平丰度表\n")
cat("  feature_tables/alpha_diversity_metrics.tsv   — α 多样性指标表\n")
cat("  feature_tables/feature_table_with_taxonomy.tsv — 附分类学特征表\n")
cat("========================================\n")
cat("脚本运行成功！\n")
