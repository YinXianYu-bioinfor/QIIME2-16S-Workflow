...
#   2. Run: Rscript QIIME2_16S_visualization_EN.R [options]
#      All parameters have defaults, override via --key=value or -X value
...
# ============================================================
# 0. Configuration - All parameters have defaults, overridable via CLI
# ============================================================
...
parse_cli_args <- function() {
  ...
  while (i <= length(args)) {
    if (args[i] %in% c("--help", "-h")) {
      ...
      cat("                                All input/output paths are derived from this.\n\n")
      cat("Data parameters:\n")
      cat("  -g, --group-col=<col>        Group column name (default: Group)\n\n")
      cat("Visualization parameters:\n")
      cat("  -n, --top-n-genera=<n>       Top N genera in heatmap (default: 30)\n")
      cat("  -y, --top-n-phyla-stacked=<n> Top N phyla in stacked barplot (default: 8)\n")
...
# ---- Input file paths (derived from export_dir) ----
...
# ---- Output subdirectories (derived from export_dir) ----
...
check_packages <- function(pkgs) {
  ...
  if (length(missing) > 0) {
    ...
    install.packages(missing, repos = "https://mirrors.tuna.tsinghua.edu.cn/CRAN", quiet = TRUE)
  }
...
# 7b2. Save alpha diversity statistics table (mean ± SD + Kruskal-Wallis + pairwise Wilcoxon p-values)
# ------------------------------------------------------------------
...
cat("  results/export/beta/beta_diversity_pcoa_weighted_unifrac.pdf   - Beta diversity Weighted UniFrac PCoA (with PERMANOVA)\n")
cat("  results/export/beta/beta_diversity_pcoa_unweighted_unifrac.pdf - Beta diversity Unweighted UniFrac PCoA (with PERMANOVA)\n")
...
cat("  results/export/feature_tables/alpha_diversity_statistics.tsv - Alpha diversity statistics (mean±SD, K-W, pairwise Wilcoxon)\n")
