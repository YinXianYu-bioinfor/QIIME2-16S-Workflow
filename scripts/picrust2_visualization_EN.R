...
# Group color palette (default npg journal palette)
...
# DESeq2 LRT differential analysis wrapper
...
# Differential heatmap (pheatmap + VST normalization)
plot_diff_heatmap <- function(dds, feature_names, sample_annot, ann_colors, title, file_path, width = 9, height = 8) {
  ...
  if (length(feat) < 2) {
    warning("Differential heatmap: fewer than 2 significant features, skipping.")
    return(NULL)
  }
...
# ---- Read pathway abundance (pathways_out) ----
...
if (file.exists(pathway_file)) {
  ...
  cat(sprintf("  Pathway abundance table: %d × %d\n", nrow(pathway), ncol(pathway)))
} else {
  cat("  ! Pathway abundance table not found, skipping Pathway analysis:", pathway_file, "\n")
  run_pathway <- FALSE
}
...
if (file.exists(nsti_file)) {
  ...
  cat(sprintf("  NSTI data: %d samples\n", nrow(nsti)))
}
...
if (run_ec && file.exists(ec_file) && file.exists(ec_meta_file)) {
  ...
  cat(sprintf("  EC abundance: %d × %d\n", nrow(ec_count), ncol(ec_count)))
}

cat("  Data reading completed.\n\n")

# ==============================================================================
# 3. NSTI Quality Assessment
# ==============================================================================
cat("[3/8] NSTI quality assessment...\n")
...
if (!is.null(nsti_meta) && nrow(nsti_meta) > 0) {
  ...
  cat("  01_NSTI_Boxplot.pdf generated\n")
} else {
  cat("  Skipping: NSTI data not available\n")
}

# ==============================================================================
# 4. KEGG Hierarchy Overview
# ==============================================================================
cat("[4/8] KEGG hierarchy overview...\n")

if (run_pathway && !is.null(kegg_l1) && nrow(kegg_l1) > 0) {
  # ---- 4a. L1 category stacked barplot ----
  ...
  cat("  02_KEGG_L1_Stackplot.pdf generated\n")

  # ---- 4b. L2 variable categories heatmap ----
  if (!is.null(kegg_l2) && nrow(kegg_l2) > 1) {
    ...
    grid::grid.draw(ph$gtable)
    dev.off()
    cat("  03_KEGG_L2_Heatmap.pdf generated\n")
  } else {
    cat("  Skipping L2 heatmap: insufficient L2 data\n")
  }
} else {
  cat("  Skipping: KEGG hierarchy data not available\n")
}

# ==============================================================================
# 5. KEGG Pathway Analysis
# ==============================================================================
cat("[5/8] Pathway analysis...\n")

if (run_pathway && !is.null(pathway) && nrow(pathway) > 0) {

  pathway_rel <- normalize_abundance(pathway)

  # ---- 5a. Top pathways faceted horizontal barplot (top 10 per group) ----
  ...
  cat("  04_Pathway_Top10_Barplot.pdf generated\n")



  # ---- 5b. Functional beta diversity (Bray-Curtis PCoA) ----
  ...
  cat("  07_Pathway_PCoA.pdf generated\n")

  # ---- 5c. DESeq2 LRT multi-group differential analysis ----
  cat("  Running DESeq2 LRT differential analysis...\n")
  ...
  n_sig <- nrow(sig_res)
  cat(sprintf("  LRT detected %d significantly differential pathways (padj < %.3f)\n", n_sig, padj_cutoff))
  ...
  cat("  08_DESeq2_LRT_Barplot.pdf generated\n")


  # ---- 5d. Volcano plot (pairwise comparison) ----
  ...
  for (cmp in cmp_grps) {
    ...
    cat(sprintf("  11_Volcano_%s_vs_%s.pdf generated\n", safe_cmp, safe_ref))
  }
} else {
  cat("  Skipping: Pathway data not available\n")
}
...
if (run_ko && !is.null(ko_count) && nrow(ko_count) > 0) {
  ...
  # ---- 6a. KO PCA ----
  # High KO dimension, using top 500 most variable KO for PCA
  ...
  cat("  12_KO_PCA.pdf generated\n")

  # ---- 6b. KO DESeq2 LRT ----
  cat("  Running KO DESeq2 LRT differential analysis...\n")
  ...
  sig_res <- lrt_res[lrt_res$padj < padj_cutoff & !is.na(lrt_res$padj), ]
  n_sig <- nrow(sig_res)
  cat(sprintf("  KO LRT detected %d significantly differential KOs (padj < %.3f)\n", n_sig, padj_cutoff))

  top_diff <- if (n_sig > 0) head(sig_res$Feature, n_top_diff) else head(lrt_res$Feature, n_top_diff)

  # LRT barplot (with descriptions)
  ...
  cat("  13_KO_LRT_Barplot.pdf generated\n")


} else {
  cat("  Skipping: KO data not available\n")
}
...
if (run_ec && !is.null(ec_count) && nrow(ec_count) > 0) {
  ...
  cat("  17_EC_Class_Stackplot.pdf generated\n")

  # ---- 7b. EC DESeq2 LRT ----
  cat("  Running EC DESeq2 LRT differential analysis...\n")
  ...
  sig_res <- lrt_res[lrt_res$padj < padj_cutoff & !is.na(lrt_res$padj), ]
  n_sig <- nrow(sig_res)
  cat(sprintf("  EC LRT detected %d significantly differential ECs (padj < %.3f)\n", n_sig, padj_cutoff))
  ...
  cat("  18_EC_LRT_Barplot.pdf generated\n")


} else {
  cat("  Skipping: EC data not available\n")
}
...
# ==============================================================================
cat("\n======================================================================\n")
cat("  PICRUSt2 multi-group visualization completed!\n")
